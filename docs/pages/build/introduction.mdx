...
class UGCCreatorService {
  ...
  async registerUGCCreator(name: string, email: string, externalId: string): Promise<string | null> {
    try {
      ...
      return data.sideshiftAccountId;
    } catch (error) {
  ...
  async processCreatorCommission(sideshiftAccountId: string, amountUsd: number): Promise<boolean> {
    try {
      const response = await fetch(`${this.baseUrl}/transfers/create`, {
        ...
        body: JSON.stringify({
          sideshiftAccountId,
          amount: amountUsd * 0.20, // 20% commission to UGC creator
          currency: 'USD',
          description: 'Soulpad Referral Bonus',
        }),
      });
      return response.ok;
    } catch (error) {
...
export const SideShiftEngine = new UGCCreatorService();
